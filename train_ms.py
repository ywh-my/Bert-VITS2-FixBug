...
## shutil 复制模型文件的函数
def copy_pth_files(src_dir, dst_dir):
    # 检查目标目录是否存在，不存在则创建
...
def run():
    ...
    model_dir = os.path.join(args.model, config.train_ms_config.model) ## 注意这里给输入model目录加了一个models子目录。
    if not os.path.exists(model_dir):
        os.makedirs(model_dir, exist_ok=True)
    hps = utils.get_hparams_from_file(args.config)
    hps.model_dir = model_dir
    print(args.base_model,model_dir)
    ## 进行底模复制：

    copy_pth_files(src_dir=args.base_model,dst_dir=model_dir)
    print("复制底模型文件。")
...
"""
# 输入三个参数：新建的配置文件，输出目录，底模存放目录
CUDA_VISIBLE_DEVICES=1,2  torchrun --nproc_per_node=2  train_ms.py -c A5_finetuned_trainingout/SSB0005_50/config.json \
-m A5_finetuned_trainingout/SSB0005_50 \
